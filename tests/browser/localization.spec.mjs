...
async function unknownMediaDuration(page) {
  await page.addInitScript(() => {
    ...
    window.hideSeekable = false;
    Object.defineProperty(HTMLMediaElement.prototype, 'duration', {
      configurable: true,
      get() { return window.reportedDuration === 'actual' ? duration.get.call(this) : window.reportedDuration; }
    });
    Object.defineProperty(HTMLMediaElement.prototype, 'seekable', {
      configurable: true,
      get() { return window.hideSeekable ? { length: 0 } : seekable.get.call(this); }
    });
  });
}

for (const platform of ['android', 'linux']) {
  test(`Napstrfy ${platform}: unknown song length stays seekable and late metadata repairs the timeline`, async ({ page }) => {
    await mockNative(page, { platform });
    await unknownMediaDuration(page);
    await page.route('**/fixture*.wav', serveAudio);
    await page.goto('http://127.0.0.1:15174');
    await page.locator('.track-open').first().click();
    const audio = page.locator('audio');
    ...
    await expect.poll(() => audio.evaluate((audio) => audio.currentTime)).toBe(30);
    await page.getByRole('button', { name: 'Forward 15 seconds', exact: true }).click();
    await expect.poll(() => audio.evaluate((audio) => audio.currentTime)).toBe(45);
    expect(await audio.evaluate((audio) => audio.paused)).toBe(true);
    // Some webviews update the property without sending durationchange again.
    await page.evaluate(() => {
      window.reportedDuration = 60;
      document.querySelector('audio').dispatchEvent(new Event('progress'));
    });
    await expect(page.locator('.timeline span')).toHaveText('0:45 / 1:00');
    await page.evaluate(() => {
      window.reportedDuration = Infinity;
      document.querySelector('audio').dispatchEvent(new Event('durationchange'));
    });
    await expect(timeline).toHaveAttribute('max', '60');
    await timeline.fill('15');
    await expect.poll(() => audio.evaluate((audio) => audio.currentTime)).toBe(15);
    expect(await page.evaluate(() => window.calls.filter((call) => call.cmd === 'cache_remote_audio').length)).toBe(1);
    // A new source must not inherit the previous song's length or seek range.
    await page.evaluate(() => {
      window.hideSeekable = true;
      window.reportedDuration = NaN;
      window.nextMediaSource = '/fixture-next.wav';
    });
    await page.locator('.track-open').first().click();
    await expect(audio).toHaveAttribute('src', /fixture-next\.wav$/);
    await expect(timeline).toBeDisabled();
    await expect(timeline).toHaveAttribute('max', '0');
    await expect(page.locator('.timeline span')).toContainText('/ —');
    await page.evaluate(() => {
      window.hideSeekable = false;
      window.reportedDuration = 'actual';
...
test('Napstrfy podcast feed duration survives unknown media length until the actual duration arrives', async ({ page }) => {
  ...
  await page.locator('.play-main').click();
  await expect(timeline).toBeEnabled();
  await expect(timeline).toHaveAttribute('max', '100');
